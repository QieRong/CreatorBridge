...
4. 每个平台内容体现明显的风格差异。
5. 支持“快速适配（本地规则）”和“智能适配（AI 语义改写）”两种内容生成模式，并保留多级格式检查、工作台 Tab 独立预览、一键复制和模拟发布。
...
| 快速适配 | 使用本地规则生成各平台适配版本，无需启动后端 | 已完成 |
| 智能适配（AI） | 通过本地代理调用 OpenRouter 模型，按原文语义改写并保留规则校验与回退 | MVP 已完成 |
...
| Node.js + Express | 本地 AI 代理，隔离 OpenRouter API Key |
| dotenv | 从本机 `.env` 读取 OpenRouter 配置 |
| OpenRouter | AI 语义适配的模型网关；模型由用户自行选择 NVIDIA 模型 ID |
...
* **依赖说明**：快速适配可直接打开 `index.html` 使用。智能适配需要执行 `npm install` 和 `npm run dev` 启动本地 Node 服务；服务端依赖仅为 Express 与 dotenv。桌面端仍为可选能力，若使用 Electron 启动或打包，也需安装 package.json 中声明的 Electron 相关依赖。
* **独立开发说明**：本项目核心功能为独立设计与实现，未套用任何第三方前/后端业务模板：
  ...
  5. **AI 适配代理** (`server.js`)：仅负责将受限长度的用户输入转发给 OpenRouter，不保存 API Key、完整正文或真实平台凭证。
...
AI 智能适配使用本地 `server.js` 调用 OpenRouter，浏览器仅向同源的 `/api/ai-adapt` 发送标题、正文、标签和已选平台。`OPENROUTER_API_KEY` 只保存在根目录 `.env`，该文件已由 `.gitignore` 排除；前端 HTML、JavaScript、Payload、草稿和发布历史均不会保存该 Key。服务端对输入长度设置上限、请求设置超时，且不会记录用户完整正文或 API Key。
...
| 模式 | 生成方式 | 适用场景 | 失败行为 |
| --- | --- | --- | --- |
| 快速适配（本地规则） | 复用 `src/js/adapters.js` 的本地规则 | 离线演示、快速预览 | 不依赖网络 |
| 智能适配（AI） | `server.js` 通过 OpenRouter 调用用户选择的 NVIDIA 模型 | 需要按语义调整标题、段落、语气与标签 | 服务不可用、超时、余额不足、JSON 非法或平台不完整时自动回退快速适配 |

AI 的职责是理解原文并按平台重新组织内容，而非机械截断；它不得编造数据、人物或结论。规则引擎仍负责硬性格式校验、标签数量控制和句子级安全截断。因此，智能适配是语义改写 MVP，不是自动真实发布功能。

图片和视频仍按原有本地素材流程处理：文件只在浏览器中预览，素材元数据会随既有 Payload 和 Mock 发布流程保留，但文件本体不会发送给 OpenRouter，也不会被 AI 识别或上传。若未来需要“看图生成文案”，应单独设计可选多模态授权、文件类型/大小限制和隐私提示，当前版本不实现该能力。

各平台 AI 改写要求如下：

* **公众号**：正式、完整、逻辑清晰，标题不超过 64 字。
* **知乎**：理性分析、问题或观点导向，标题不超过 50 字，标签最多 5 个。
* **B站**：年轻化视频简介或专栏风格，标题不超过 80 字。
* **小红书**：自然口语化，标题不超过 20 字、正文不超过 1000 字，可适量使用 Emoji，不使用夸张模板词。
* **微博**：短平快，正文优先控制在 140 字左右，标签使用 `#话题#` 格式。
...
> 静态服务器或直接双击页面仅能使用“快速适配”。若要使用“智能适配”，请按下面的 Node 服务方式启动。

### 🤖 启动 OpenRouter AI 智能适配

```bash
# 1. 安装依赖
npm install

# 2. 在项目根目录复制配置模板为 .env（PowerShell）
Copy-Item .env.example .env

# 3. 编辑 .env，填写你自己的 OpenRouter 凭证和 NVIDIA 模型 ID
# OPENROUTER_API_KEY=你的_OpenRouter_API_Key
# OPENROUTER_MODEL=你在_OpenRouter_模型页选择的_NVIDIA_模型_ID

# 4. 启动本地服务，并访问 http://localhost:3000
npm run dev
```

OpenRouter 使用兼容 Chat Completions 的接口；模型 ID 应以 OpenRouter 当前模型页显示的实际值为准。项目不会替你提供、保存或上传 API Key。接口格式可参考 [OpenRouter API 文档](https://openrouter.ai/docs/api_reference/overview)。
...
3. **选择模式并适配**：选择“快速适配（本地规则）”或“智能适配（AI）”，点击“一键适配”。中栏会生成各平台专属 Tab 页；智能适配会在 AI 异常时提示并自动切回本地规则。点击不同 Tab，可分别审阅对应平台的排版效果、字数预估及格式校验建议。
4. **输出与发布**：
...
2. AI 智能适配目前是本地代理 MVP：需要用户自备 OpenRouter Key 和可用 NVIDIA 模型，且不同模型的语义质量、JSON 遵从性和额度情况会影响实际体验。
