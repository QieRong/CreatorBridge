...
| 平台选择 | 公众号、知乎、B站、小红书、微博 | 已完成 |
| 本地素材管理 | 支持本地图片、视频选择及元数据读取，支持本地预览，不上传 | 已完成 |
| 一键适配 | 自动生成各平台的适配版本 | 已完成 |
...
| 接口预留配置 | 支持中转 API 地址、鉴权方式及端点配置 | 当前仅作 UI 预留和 Payload 结构展示 |
| Payload 导出 | 导出标准的 PublishPayload JSON 载荷文件，含素材元数据 | 已完成 |
...
* **模式定义**：在本地运行的模拟发布验证模式。
* **业务行为**：点击发布后，系统会为选中的每个平台生成独立的发布任务，加入任务队列并可视化展示“待发布、校验中、发布中、成功、失败”状态流转。支持对失败任务进行重试，完整批次将写入本地 localStorage 发布历史中。
* **网络请求**：不发起任何真实网络请求。
...
3. **非敏感参数暂不持久化**：API Base URL 预留输入框本阶段不保存到 localStorage，仅作为自建后端中转接口的地址预留展示，避免凭证残留。
4. **零硬编码承诺**：前端源代码中不包含任何社交平台 API 的硬编码 Key，不进行任何非官方爬虫登录，不强制写入 Cookie。
5. **推荐最佳实践**：我们倡导“**前端不留密钥**”的行业安全规范。创作者应当通过自建的后端代理中转服务（在服务端进行安全的密钥存储、请求签名与重试机制），前端只负责向该中转服务发送包含元数据的标准 Payload，暂不发起真实网络请求，后续可扩展。
...
1. 在输入区填写文章标题、正文、标签。
2. （可选）点击添加本地图片或视频素材，并在右侧预览其加载状态。
3. 在平台选择区勾选需要适配的平台（公众号、知乎、B站、小红书、微博）。
4. 点击「一键适配」按钮，系统自动生成各平台的适配版本和 Payload 载荷。
5. 在预览区查看各平台内容差异，确认格式检查结果。
6. 点击「模拟发布」触发发布任务队列，观察各平台任务的状态流转（成功/失败），可对失败任务点击重试。
